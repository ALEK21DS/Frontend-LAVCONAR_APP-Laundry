...
export const BottomNav: React.FC<BottomNavProps> = ({ active, onNavigate }) => {
  ...
  useEffect(() => {
    ...
    if (radialOpen) {
      ...
      // cerrar en orden inverso (derecha->arriba->izquierda)
      Animated.stagger(50, [to3, to2, to1]).start();
    }
            ...
            <View key={tab.key} className="items-center justify-center" style={{ zIndex: 60 }}>
              ...
              {radialOpen && (
                <View
                  ...
                  style={{ position: 'absolute', left: 0, right: 0, bottom: 0, top: -900, zIndex: 40 }}
                  // @ts-ignore responder props
                  onStartShouldSetResponder={() => true}
                  onResponderRelease={() => setRadialOpen(false)}
                />
              )}
              ...
              {radialOpen && (
                <>
                  {/* Arriba (Prenda) color igual a Dashboard (#10B981) */}
                  <Animated.View style={{ position: 'absolute', top: -115, transform: [{ scale: animTop }], opacity: animTop, zIndex: 50 }}>
                  ...
                  {/* Izquierda (Guía) color igual a Dashboard (#F59E0B) */}
                  <Animated.View style={{ position: 'absolute', top: -70, left: -75, transform: [{ scale: animLeft }], opacity: animLeft, zIndex: 50 }}>
                  ...
                  <Animated.View style={{ position: 'absolute', top: -70, right: -75, transform: [{ scale: animRight }], opacity: animRight, zIndex: 50 }}>
        ...
        if (isGuides) {
          return (
            <View key={tab.key} className="flex-1 items-center" style={{ position: 'relative' }}>
              ...
              {/* Radial pequeño: arriba = Guías, diagonal = Prendas */}
              {guidesRadialOpen && (
                <>
                  <Animated.View style={{ position: 'absolute', bottom: 28, transform: [{ translateY: guidesAnimUp.interpolate({ inputRange: [0,1], outputRange: [0,-48] }) }, { scale: guidesAnimUp }], opacity: guidesAnimUp, zIndex: 50 }}>
                    <TouchableOpacity
                  ...
                  <Animated.View style={{ position: 'absolute', bottom: 22, transform: [{ translateX: guidesAnimDiag.interpolate({ inputRange: [0,1], outputRange: [0,-44] }) }, { translateY: guidesAnimDiag.interpolate({ inputRange: [0,1], outputRange: [0,-30] }) }, { scale: guidesAnimDiag }], opacity: guidesAnimDiag, zIndex: 50 }}>
                    <TouchableOpacity
                      className="w-12 h-12 rounded-full items-center justify-center"
